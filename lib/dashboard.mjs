...
const simplifyApplication = (entry = {}) => {
    const rxBytes = toNumber(entry.rx_bytes) || 0;
    const txBytes = toNumber(entry.tx_bytes) || 0;
    const total = rxBytes + txBytes;
    const name = entry.app || entry.cat || entry.app_name || entry.application || 'Unknown';
    return {
        name,
        category: entry.cat || entry.category || null,
        rxBytes,
        txBytes,
        total,
    };
...
const fetchTopApplications = async () => {
    try {
        const response = await requestJson(`/proxy/network/api/s/${config.site}/stat/dpi`, {
            query: {
                type: 'app',
                sort_by: 'rx_bytes',
                sort_order: 'desc',
...
const fetchSnapshot = async () => {
    const [metrics, clients, applications] = await Promise.all([
        fetchWanHealth(),
        fetchActiveClients(),
