...
const decodeResponseBody = (input) => {
    const buffer = Buffer.isBuffer(input) ? input : Buffer.from(input || []);
    if (!buffer.length) { return ''; }

    const isGzip = buffer.length >= 3
        && buffer[0] === 0x1f
        && buffer[1] === 0x8b
        && buffer[2] === 0x08;
    ...
    try {
        ...
        utilitas.throwError(`UniFi API returned invalid gzip. ${error.message}`, 502);
    }
};

const parseJsonBody = (text = '', contentType = '') => {
    const body = String(text || '').trim();
    if (!body) { return {}; }

    const jsonHeader = contentType.toLowerCase().includes('application/json');
    const jsonBody = body.startsWith('{') || body.startsWith('[');
    if (!jsonHeader && !jsonBody) { return {}; }

    try {
        return JSON.parse(body);
    } catch (error) {
        utilitas.throwError(`UniFi API returned invalid JSON. ${error.message}`, 502);
    }
};

const listPayload = (response, ...keys) => {
...
const fetchActiveClients = async () => {
    try {
        ...
        const list = listPayload(response, 'data');
        const items = list
            .map(simplifyClient)
...
const simplifyDevice = (device = {}) => {
    const rxBytes = toNumber(device?.stat?.rx_bytes) || toNumber(device?.rx_bytes);
    const txBytes = toNumber(device?.stat?.tx_bytes) || toNumber(device?.tx_bytes);
    return {
        name: device.name || device.model || device.mac || 'Unknown',
        type: device.type || device.device_type || 'unknown',
        mac: device.mac || null,
        ip: device.ip || device.ip_address || null,
        model: device.model || device.product_type || device.model_intern || null,
        state: device.state,
        version: device.version || device.fw_version || null,
        rxBytes,
        txBytes,
    };
};
...
const fetchDevices = async () => {
    try {
        const response = await requestJson(`/proxy/network/api/s/${config.site}/stat/device`);
        const list = listPayload(response, 'data');
        const items = list.map(simplifyDevice);
        return { items };
    } catch (error) {
        return { items: [], error };
    }
...
const fetchWanHealth = async () => {
    ...
    const payload = listPayload(response, 'data');
...
const fetchSnapshot = async () => {
    const [metrics, clients, devices] = await Promise.all([
        fetchWanHealth(),
        fetchActiveClients(),
        fetchDevices(),
    ]);
